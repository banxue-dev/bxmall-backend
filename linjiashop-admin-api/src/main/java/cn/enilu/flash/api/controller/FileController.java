...
public class FileController extends BaseController {
    ...
    @ResponseBody
    public Ret ossFileUpload(@RequestParam("file") MultipartFile file,HttpServletRequest request) {
        try {
            if (file.isEmpty()) {
                return Rets.failure("文件是空的");
            }
            String hz= file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf("."));
            if(limitFile.indexOf(hz.toLowerCase())<0) {
            	return Rets.failure("文件异常，只能上传"+limitFile+"的格式文件");
            }
            String fileName = System.currentTimeMillis() + UUID.randomUUID().toString()+hz;
            FileUtil.writeBytes(file.getBytes(), fileService.getCfgValue(CfgKey.SYSTEM_FILE_UPLOAD_PATH) + fileName);
//            FileUtil.writeBytes(file.getBytes(), hostPath + fileName);
            String url=webImgPath.replaceAll("\\*\\*",fileName);
            FileInfo f=fileService.save2(fileName, url);
            return Rets.success(f);
        } catch (Exception e) {
            e.printStackTrace();
            logger.warn("上传文件失败！");
            return Rets.failure("上传文件失败");
