...
public class FileService extends BaseService<FileInfo,Long,FileInfoRepository> {
     ...
     * @return
     */
    public FileInfo save2(String originalFileName,String trueName){
    	try {
    		FileInfo fileInfo = new FileInfo();
    		fileInfo.setCreateTime(new Date());
    		fileInfo.setCreateBy(JwtUtil.getUserId());
    		fileInfo.setOriginalFileName(originalFileName);
    		fileInfo.setRealFileName(trueName);
    		fileInfoRepository.save(fileInfo);
    		return fileInfo;
    	} catch (Exception e) {
    		e.printStackTrace();
    ...
    public String getCfgValue(String key) {
    	return cfgService.getCfgValue(key) ;
    }
    public FileInfo getByName(String fileName) {
        FileInfo fileInfo =  get(SearchFilter.build("realFileName",fileName));
        fileInfo.setAblatePath(cfgService.getCfgValue(CfgKey.SYSTEM_FILE_UPLOAD_PATH) + File.separator+fileInfo.getOriginalFileName());
        return fileInfo;
    }
}
